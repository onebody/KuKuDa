...
exports.executionService = {
    ...
    async executeWorkflow(workflowId, userId) {
        ...
        // 异步执行工作流
        exports.executionService.runExecution(execution.id, workflow.nodes, workflow.connections, userId).catch(console.error);
        return execution;
    },
    /**
     * 运行执行（核心执行引擎 - 重构版）
     ...
     * @param userId 用户ID
     */
    async runExecution(executionId, nodes, connections, userId) {
        ...
        try {
            ...
            while (queue.length > 0) {
                ...
                // 执行节点
                const result = await this.executeNode(node, nodeResults, executionContext);
    ...
    getAdapter(nodeType) {
        // 从适配器注册表创建实例
        const instance = nodeAdapters_1.adapterRegistry.createInstance(nodeType);
        return instance;
    },
    /**
     * 获取节点输入（从上游节点结果）
     ...
     * @returns 节点输入
     */
    getNodeInput(nodeId, previousResults, inputPorts) {
        const input = {};
     ...
     * @returns 解析后的文本
     */
    resolveVariables(text, nodeResults) {
