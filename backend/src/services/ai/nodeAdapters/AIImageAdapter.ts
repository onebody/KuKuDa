 ...
 * 继承 BaseNodeAdapter，实现AI绘图节点的具体逻辑
 */
export class AIImageAdapter extends BaseNodeAdapter {
  nodeType = 'AI_IMAGE'
  ...
  validate(input: NodeInput, config: Record<string, any>): ValidationResult {
    ...
    if (input) {
      // 检查是否有有效的输入数据
      const hasValidInput = Object.values(input).some(
        (data) => data && (data.text || data.imageUrls || data.json)
      )
   ...
   * @returns 提取的文本
   */
  private extractTextFromInput(input: NodeInput): string {
    // 优先使用 'prompt' 句柄
  ...
  private resolveVariables(text: string, variables: Record<string, any>): string {
    ...
    return text.replace(
      ...
      (match, nodeId, handleId, field) => {
        ...
        console.warn(
          `[AIImageAdapter] 未找到节点 ${nodeId} 的句柄 ${handleId}`
        )
        return match
      }
    )
   ...
   * @returns API请求参数
   */
  private buildRequestParams(prompt: string, config: Record<string, any>): any {
    const params: any = {
      prompt,
  ...
  private async callAIAPI(
    params: any,
    context: ExecutionContext
  ): Promise<{ imageUrls: string[]; seed: number }> {
    // 模拟API调用延迟
